...
import matplotlib
matplotlib.use('Agg')  # Backend no-GUI para entornos sin display
...
def crear_usuario():
    ...
    registrar_actividad(
        "creacion", f"Nuevo {rol_solicitado} registrado: {usuario_nombre}")
    return redirect(url_for("admin.admin_panel"))
...
def eliminar_usuario(id):
    ...
    if usuario:
        ...
        registrar_actividad(
            "eliminacion", f"Acceso revocado al usuario: {usuario['nombre_usuario']}")
...
def asignar_tarea():
    ...
    # === ESCUDO PROTECTOR BIOMÉTRICO ===
    # Solo se activa si la tarea que quieren crear es "Alta"
    prioridad = request.form.get("prioridad", "Media")
    asignados = request.form.getlist("usuarios")

    if prioridad == "Alta":
        for usuario_id_str in asignados:
            ...
            if ultimo_bpm and ultimo_bpm.get("ritmo_cardiaco_promedio", 0) >= 100:
                flash(
                    f"⚠️ OPERACIÓN DENEGADA: El sistema ha detectado sobrecarga biológica ({ultimo_bpm['ritmo_cardiaco_promedio']} BPM) en uno de los empleados seleccionados. No es ético asignarle tareas de prioridad Alta en este momento.",
                    "error"
                )
...
def eliminar_tarea(tarea_id):
    ...
    if tarea:
        ...
        registrar_actividad(
            "eliminacion", f"Tarea eliminada permanentemente: '{tarea.get('titulo', 'Sin título')}'")
...
def admin_estadisticas():
    ...
    tareas_pendientes = tareas_col.count_documents(
        {"estado": "Pendiente", **filtro_empresa})
    tareas_completadas = tareas_col.count_documents(
        {"estado": "Completada", **filtro_empresa})

    # Métricas adicionales para dashboard
    carga_por_usuario = round(
        tareas_pendientes / total_usuarios, 2) if total_usuarios else 0.0
    # placeholder: tareas pendientes como 'en riesgo'
    tareas_en_riesgo = tareas_pendientes
    tasa_eficiencia = round(
        (tareas_completadas / total_tareas) * 100, 1) if total_tareas else 0.0
    ...
    # Coeficiente constante (El equipo resuelve el 15% de la carga al día)
    k = 0.15
...
def admin_seguridad():
    ...
    logs_recientes = list(db["logs_seguridad"].find().sort(
        "fecha_intento", -1).limit(50))
...
def resetear_password(id):
    ...
    if usuario:
        usuarios_col.update_one(
            {"_id": ObjectId(id)},
            {"$set": {"contrasena": generate_password_hash(
                nueva_clave), "solicitud_reset": False}}
        )
        registrar_actividad(
            "alerta", f"Contraseña restablecida por administrador para: {usuario['nombre_usuario']}")
...
def admin_panel():
    ...
    historial = list(actividades_col.find(
        filtro_empresa).sort("fecha", -1).limit(50))
    logs_seguridad = list(db["logs_seguridad"].find().sort(
        "fecha_intento", -1).limit(20))

    usuarios = list(usuarios_col.find(filtro_usuarios))

    # === MOTOR DEL RADAR DE SALUD (ESCALABLE) ===
    for u in usuarios:
        id_empleado = u["_id"]  # <--- Nombre de variable más claro

        nombre_empleado = u.get("nombre", u.get(
            "usuario", u.get("correo", "Empleado")))

        print(f"🔍 Buscando corazón para {nombre_empleado} (ID: {id_empleado})")

        ultimo_registro = db["registro_biometrico"].find_one(
        )

        if ultimo_registro:
            bpm_encontrado = ultimo_registro["ritmo_cardiaco_promedio"]
            print(
                f"   ✅ ¡ÉXITO! Encontré {bpm_encontrado} BPM para {nombre_empleado}")
            u["bpm_actual"] = bpm_encontrado
        else:
            print(f"   ❌ FALLO: No encontré registros para {nombre_empleado}")
            u["bpm_actual"] = None
    # ========================================================
    ...
    return render_template(
        ...
        usuarios=usuarios,
        tareas=list(tareas_col.find(filtro_empresa).sort(
            "fecha_creacion", -1).limit(30)),
...
def descargar_reporte():
    ...
    try:
        ...
        filtro_empresa = {"empresa_id": ObjectId(
        ...
        pendientes = tareas_col.count_documents(
            {"estado": "Pendiente", **filtro_empresa})

        completadas = tareas_col.count_documents(
            {"estado": "Completada", **filtro_empresa})

        tasa_eficiencia = (completadas / total_tareas *
                           100) if total_tareas > 0 else 0

        print(
            f"Datos obtenidos: usuarios={total_usuarios}, tareas={total_tareas}, pendientes={pendientes}")
        ...
        pdf.cell(0, 8, "REPORTE EJECUTIVO - GESTION DE CARGA MENTAL",
                 ln=True, align='C')
        ...
        pdf.cell(
            0, 4, f"Fecha de emision: {fecha_actual}", ln=True, align='R')
        ...
        metrics = [
            ...
            ("Tareas Pendientes", str(pendientes), "Atencion",
             "Subiendo" if pendientes > completadas else "Estable"),


            ("Tareas Completadas", str(completadas), "Excelente", "Estable"),


            ("Eficiencia General", f"{tasa_eficiencia:.1f}%", "Buena" if tasa_eficiencia >=
             70 else "Mejorable", "Mejorando" if tasa_eficiencia >= 70 else "Estable")
        ...
        for i, (indicador, valor, estado, tendencia) in enumerate(metrics):
            ...
            if pendientes > completadas:
                ...
                pdf.cell(5, 4, "-", 0, 0)
                pdf.cell(
                    0, 4, "Se recomienda optimizar la distribucion de carga de trabajo para reducir tareas pendientes.", ln=True)
                pdf.set_text_color(0, 0, 0)
            else:
                pdf.set_text_color(0, 150, 50)
                pdf.cell(5, 4, "-", 0, 0)
                pdf.cell(
                    0, 4, "Excelente balance en la gestion de tareas. Mantener las practicas actuales.", ln=True)
                pdf.set_text_color(0, 0, 0)
            ...
            if tasa_eficiencia < 70:
                ...
                pdf.cell(5, 4, "-", 0, 0)
                pdf.cell(
                    0, 4, "Implementar estrategias para mejorar la tasa de eficiencia operativa.", ln=True)
                pdf.set_text_color(0, 0, 0)
            else:
                pdf.set_text_color(0, 150, 50)
                pdf.cell(5, 4, "-", 0, 0)
                pdf.cell(
                0, 4, "La eficiencia operativa se encuentra en niveles optimos.", ln=True)
            ...
            pdf.cell(
                0, 3, "Tassflow Analytics - Sistema de Gestion de Carga Mental", ln=True, align='C')

            pdf.cell(
                0, 3, "Documento confidencial - Para distribucion interna unicamente", ln=True, align='C')

            pdf.cell(
                0, 3, f"Pagina 1 de 1 - Generado el {fecha_actual}", ln=True, align='C')
            ...
            ruta_pdf = os.path.join(
                uploads_dir, "Reporte_Ejecutivo_Tassflow.pdf")

            print(f"Guardando PDF en: {ruta_pdf}")
            ...
            registrar_actividad(
                "info", "Reporte PDF ejecutivo descargado por administrador")

            print("Actividad registrada, enviando archivo...")

            return send_file(ruta_pdf, as_attachment=True, mimetype='application/pdf', download_name='Reporte_Ejecutivo_Tassflow.pdf')

    except Exception as e:
        print(f"ERROR CRITICO EN PDF: {str(e)}")
        import traceback
        traceback.print_exc()
...
def descargar_reporte_seguridad():
    ...
    alertas_comunes = sorted(alertas_count.items(),
    ...
    ips_mas_activas = sorted(
        ips_count.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    metrics = [
        ...
        ("Ultima Hora", str(logs_ultima_hora),
         "Normal" if logs_ultima_hora < 10 else "Atencion", "Estable"),
        ("Ultimas 24h", str(logs_ultimas_24h),
         "Normal" if logs_ultimas_24h < 100 else "Atencion", "Estable"),
    ...
    registrar_actividad(
        "info", "Reporte PDF de seguridad descargado por administrador")
...
def descargar_reporte_estadisticas():
    ...
    tasa_eficiencia = (tareas_completadas / total_tareas *
                       100) if total_tareas > 0 else 0
    carga_promedio = (
        total_tareas / total_usuarios) if total_usuarios > 0 else 0
    ...
    fig, ax = plt.subplots(figsize=(4, 3))
    wedges, texts = ax.pie(valores, labels=labels,
                           colors=colores, wedgeprops=dict(width=0.4))
    ...
    data = [
        [f'Carga/Usuario', f'{carga_promedio:.1f}',
            'Optimo' if carga_promedio <= 5 else 'Atención', 'Estable'],
        [f'Tareas Totales', f'{total_tareas}', 'Activo', 'Creciente'],
        [f'En Riesgo', f'{tareas_pendientes}', 'Atención' if tareas_pendientes > tareas_completadas else 'Normal',
            'Descendente' if tareas_pendientes < tareas_completadas else 'Estable'],
        [f'Tasa Eficiencia', f'{tasa_eficiencia:.1f}%', 'Excelente' if tasa_eficiencia >=
            80 else 'Buena' if tasa_eficiencia >= 70 else 'Mejorable', 'Mejorando' if tasa_eficiencia >= 70 else 'Estable'],
    ]
    ...
    if tareas_pendientes > tareas_completadas:
        recomendaciones.append(
            '- Priorizar la resolución de tareas pendientes para mejorar la eficiencia.')

    if tasa_eficiencia < 70:
        recomendaciones.append(
            '- Implementar estrategias para mejorar la tasa de eficiencia operativa.')
    else:
        recomendaciones.append(
            '- Mantener las prácticas actuales que han demostrado ser efectivas.')

    if carga_promedio > 5:
        recomendaciones.append(
            '- Considerar redistribuir la carga de trabajo entre más colaboradores.')
    else:
        recomendaciones.append(
            '- La distribución actual de carga por usuario es óptima.')

    recomendaciones.append(
        '- Continuar monitoreando los indicadores para mantener el rendimiento.')
    ...
    registrar_actividad(
        "info", "Reporte PDF de estadísticas descargado por administrador")
...
def dueno_panel():
    ...
    total_admins = usuarios_col.count_documents(
        {"rol": "admin", **filtro_empresa})
    total_empleados = usuarios_col.count_documents(
        {"rol": "usuario", **filtro_empresa})
...
def descargar_bienvenida():
    ...
    empresa = db.empresas.find_one(
        {"_id": ObjectId(empresa_id)}) if empresa_id else None
    ...
    pdf.cell(
        0, 6, f"Documento confidencial preparado para: {nombre_empresa} - {fecha_actual}", ln=True)
    pdf.ln(10)
    ...
    pdf.ln(2)
    pdf.cell(0, 4, "Tassflow Analytics - Software as a Service",
             ln=True, align='C')
    pdf.cell(0, 4, "Aislamiento de base de datos activado - Encriptación AES-256",
             ln=True, align='C')
    ...
    registrar_actividad(
        "creacion", f"Director {nombre_director} generó su manual de bienvenida.")
